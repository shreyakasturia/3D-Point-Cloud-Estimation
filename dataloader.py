...
class Loader():
	def __init__(self,opt,loadNovel=True,loadFixedOut=False,loadTest=False):
		...
		listFile = data_folder + "{0}_{1}.list".format(opt.category,"test" if loadTest else "train")
		self.CADs = []
	...
	def loadChunk(self,opt,loadRange=None):
		...
		for c in range(chunkSize):
			CAD = self.CADs[idx[c]]
			data["image_in"][c] = np.load(data_folder + "{0}_inputRGB/{1}.npy".format(opt.category,CAD))/255.0
			if self.loadNovel:
				rawData = scipy.io.loadmat(data_folder + "{0}_depth/{1}.mat".format(opt.category,CAD))
				depth = rawData["Z"]
				trans = rawData["trans"]
				mask = depth!=0
				...
				data["trans"][c] = trans
			if self.loadFixedOut:
				rawData_fixed = scipy.io.loadmat(data_folder + "{0}_depth_fixed{1}/{2}.mat".format(opt.category,opt.outViewN,CAD))
